...
class ContrastCELoss(nn.Module, ABC):
    ...
    def forward(self, seg, embedding, target, with_embed=False):
        ...
        embedding = F.interpolate(input=embedding, size=(h, w), mode='bilinear', align_corners=True)

        # target: [B, 1, H, W] float → [B, H, W] long (0/1 类别标签)
        target = target.squeeze(1).long()

        # seg 是单通道 logits，用 sigmoid 阈值化得到二值预测
        predict = (torch.sigmoid(seg) > 0.5).squeeze(1).long()
